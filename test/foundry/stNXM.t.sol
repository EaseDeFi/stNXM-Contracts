...
contract arNXMValultOldTest is Test {
    ...
    uint256 forkBeforePause;
    uint256 currentFork;
    uint256 forkBeforeNxmUpgrade;
    ...
    address implAddress;
    address[] riskPools = [0x462340b61e2ae2C13f01F66B727d1bFDc907E53e, 0xed9915e07aF860C3263801E223C9EaB512EB7C09];
    uint256[] tokenIds = [3, 4];
    ...
    function setUp() public {
        arNXMVaultProxy = IarNXMVault(0x1337DEF1FC06783D4b03CB8C1Bf3EBf7D0593FC4);
        currentFork = vm.createFork("https://mainnet.infura.io/v3/0c7537c516c74815abb1b4d3ad076a2e", 16846175);

        forkBeforeNxmUpgrade =
            vm.createFork("https://mainnet.infura.io/v3/0c7537c516c74815abb1b4d3ad076a2e", 16799100 - (5 * 60 * 24));
    ...
    function depositNXM(uint256 depositAmt, address user) public {
        vm.startPrank(user);
        // approve wnxm
    ...
    function withdrawWNXM(uint256 amount, address user, bool payFee) public {
        vm.startPrank(user);
    ...
    function stakeNxm(uint256 amount, address poolAddress, uint256 trancheId, uint256 requestTokenId) public {
        vm.startPrank(multisig);
        arNXMVaultProxy.stakeNxm(amount, poolAddress, trancheId, requestTokenId);
        vm.stopPrank();
    }
    ...
    function testUpdateImplementation() public view {
        ...
        require(implementation == implAddress, "implementation is still address 0");
    }
    ...
    function testInitializedValues() public {
        ...
        require(arNXMVaultProxy.stakingNFT() == address(stakingNFT), "staking nft not setup");
    ...
    function testTokenValues() public {
        ...
        uint256 nxmValueBefore = arNXMVaultProxy.nxmValue(1e18);
        uint256 arNxmValueBefore = arNXMVaultProxy.arNxmValue(1e18);
        vm.selectFork(currentFork);
        initializeV2();
        uint256 nxmValueAfter = arNXMVaultProxy.nxmValue(1e18);
        uint256 arNxmValueAfter = arNXMVaultProxy.arNxmValue(1e18);
        // since reward nxm is added to the vault nxmValue after should be
        // less than before
        require(nxmValueAfter < nxmValueBefore, "value should increase after collect reward");
        // for 1 unit of nxm user will get more arnxm
        require(arNxmValueAfter > arNxmValueBefore, "value should increase after collect reward");
    }

    function testWNXMDeposit() public {
        initializeV2();
        uint256 depositAmt = 10e18;
        uint256 expectedArNXMAmt = arNXMVaultProxy.arNxmValue(depositAmt);
        uint256 arNXMBalBefore = arNXM.balanceOf(wnxmWhale);
        // deposit to vault
        depositWNXM(depositAmt, wnxmWhale);

        uint256 arNXMBalAfter = arNXM.balanceOf(wnxmWhale);

        require((arNXMBalAfter - arNXMBalBefore) >= expectedArNXMAmt, "minted value off");
    }
    ...
    function testNXMDeposit() public {
        ...
        uint256 depositAmt = 10e18;
        uint256 expectedArNXMAmt = arNXMVaultProxy.arNxmValue(depositAmt);
        uint256 arNXMBalBefore = arNXM.balanceOf(nxmWhale);
        // deposit to vault
        depositNXM(depositAmt, nxmWhale);

        uint256 arNXMBalAfter = arNXM.balanceOf(nxmWhale);
    ...
    function testWithdrawWithFee() public {
        ...
        uint256 withdrawAmt = arNXM.balanceOf(user);
        bool withFee = true;

        uint256 wnxmBalBefore = wNXM.balanceOf(user);
        uint256 arnxmBalBefore = arNXM.balanceOf(user);
        withdrawWNXM(withdrawAmt, wnxmWhale, withFee);
        uint256 arnxmBalAfter = arNXM.balanceOf(user);
        uint256 wnxmBalAfter = wNXM.balanceOf(user);

        // check difference in arnxm balance
        require(arnxmBalBefore - arnxmBalAfter == withdrawAmt, "arnxm bal diff failed");
    ...
    function testWithdrawWithoutFee() public {
        ...
        uint256 whaleArNxmBal = arNXM.balanceOf(wnxmWhale);
        bool withFee = false;
        uint256 vaultsArNxmBalBefore = arNXM.balanceOf(wnxmWhale);
        uint256 totalPendingBefore = arNXMVaultProxy.totalPending();

        withdrawWNXM(whaleArNxmBal, wnxmWhale, withFee);
        uint256 vaultsArNxmBalAfter = arNXM.balanceOf(wnxmWhale);
        uint256 totalPendingAfter = arNXMVaultProxy.totalPending();

        require((vaultsArNxmBalBefore - vaultsArNxmBalAfter) == whaleArNxmBal, "arnxm transfer to vault error");

        require(
            (totalPendingAfter - totalPendingBefore) == arNXMVaultProxy.nxmValue(whaleArNxmBal), "pending not updated"
        );
    }
    ...
    function testFinalizeWithdrawal() public {
        ...
        // deposit to vault
        depositWNXM(10e18, wnxmWhale);
        uint256 withdrawAmt = arNXM.balanceOf(wnxmWhale);
        bool withFee = false;
        uint256 expectedWNXM = arNXMVaultProxy.nxmValue(withdrawAmt);
        withdrawWNXM(withdrawAmt, wnxmWhale, withFee);
        uint256 wnxmBalBefore = wNXM.balanceOf(wnxmWhale);
        uint256 arnxmBalBefore = arNXM.balanceOf(address(arNXMVaultProxy));
        // fast forward 7 days
        vm.warp(block.timestamp + 7 days);
        finalizeWithdrawal(wnxmWhale);
        uint256 arnxmBalAfter = arNXM.balanceOf(address(arNXMVaultProxy));
    ...
    function testCollectReward_success() public {
        initializeV2();
        uint256 lastRewardBefore = arNXMVaultProxy.lastReward();
        uint256 lastRewardCollectedBefore = arNXMVaultProxy.lastRewardTimestamp();
        vm.warp(block.timestamp + 7 days);
        vm.prank(address(1234), address(1234));
        arNXMVaultProxy.getRewardNxm();
        vm.stopPrank();
        uint256 lastRewardAfter = arNXMVaultProxy.lastReward();
        uint256 lastRewardCollectedAfter = arNXMVaultProxy.lastRewardTimestamp();
    ...
    function testStakeNXM() public {
        ...
        uint256 vaultNXMBalBefore = nxm.balanceOf(address(arNXMVaultProxy));
        uint256 aumBefore = arNXMVaultProxy.aum();

        // deposit to staking pool
        stakeNxm(amountToStake, riskPools[0], trancheId, tokenIds[0]);

        uint256 vaultNXMBalAfter = nxm.balanceOf(address(arNXMVaultProxy));
        uint256 aumAfter = arNXMVaultProxy.aum();
    ...
    function testStakeNXMAndGetNewNFT() public {
        ...
        uint256 trancheId = 217;
        uint256 amountToStake = 1000e18;
        uint256 stakingNFTBalBefore = stakingNFT.balanceOf(address(arNXMVaultProxy));
        ...
        uint256 stakingNFTBalAfter = stakingNFT.balanceOf(address(arNXMVaultProxy));

        // should mint new nft
        require((stakingNFTBalAfter - stakingNFTBalBefore) == 1, "nft not minted");
        uint256 mintedNFTTokenId = stakingNFT.totalSupply();
        require(
            stakingNFT.ownerOf(mintedNFTTokenId) == address(arNXMVaultProxy), "arNXM vault should be owner of new nft"
        );
        ...
        uint256 newTokenId = arNXMVaultProxy.tokenIds(2);
    ...
    function testRemoveTokenId() public {
        ...
        uint256 tokenIdAtIndex0Before = arNXMVaultProxy.tokenIds(0);
        ...
        uint256 tokenIdAtIndex0After = arNXMVaultProxy.tokenIds(0);
