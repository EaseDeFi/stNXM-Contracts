...
contract arNXMVault is Ownable, ERC721TokenReceiver {
    ...
    event WithdrawRequested(
        address indexed user, uint256 arAmount, uint256 nAmount, uint256 requestTime, uint256 withdrawTime
    );
     ...
     *
     */
    function initializeV2(IStakingNFT _stakingNFT, uint256[] memory _tokenIds, address[] memory _riskPools)
        external
        onlyOwner
    {
    ...
    function withdraw(uint256 _arAmount, bool _payFee) external {
        require((block.timestamp - withdrawalsPaused) > pauseDuration, "Withdrawals are temporarily paused.");

        // This amount must be determined before arNxm burn.
        uint256 nAmount = nxmValue(_arAmount);

        require((totalPending + nAmount) <= nxm.balanceOf(address(this)), "Not enough NXM available for withdrawal.");
        ...
        if (_payFee) {
            ...
            emit WithdrawRequested(msg.sender, _arAmount, nAmount, block.timestamp, block.timestamp + withdrawDelay);
        }
    }
     ...
     *
     */
    function _getRewardsNxm(address _poolAddress, uint256 _tokenId) internal returns (uint256 reward) {
        IStakingPool pool = IStakingPool(_poolAddress);

        (, reward) = pool.withdraw(_tokenId, false, true, _getActiveTrancheIds());
    }
     ...
     *
     */
    function withdrawNxm(address _poolAddress, uint256 _tokenId, uint256[] memory _trancheIds) external onlyOwner {
     ...
     *
     */
    function unstakeNxm(uint256 _tokenId, uint256[] memory _trancheIds) external onlyOwner {
    ...
    function _stakeNxm(uint256 _amount, address _poolAddress, uint256 _trancheId, uint256 _requestTokenId) internal {
        ...
        require((reserveAmount + totalPending + _amount) <= balance, "Not enough NXM");
    ...
    function rescueToken(address token) external onlyOwner {
        require(
            token != address(nxm) && token != address(wNxm) && token != address(arNxm), "Cannot rescue NXM-based tokens"
        );
        uint256 balance = IERC20(token).balanceOf(address(this));
        IERC20(token).safeTransfer(msg.sender, balance);
    }

    function transferERC721Token(address to, address tokenAddress, uint256 tokenId) external onlyOwner {
        ...
        require(tokenAddress != address(stakingNFT), "cannot transfer stakingNFT");
